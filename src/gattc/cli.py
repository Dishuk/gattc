...
def _compile_schema(
    ...
    """Compile a single schema file. Returns list of generated files.
    ...
    if generate_docs and effective_docs_dir:
        ...
        html_path = docs_gen.generate(s, docs_output)
...
def _compile_combined_mode(
    ...
    if generate_docs and docs_dir:
        from .generators import docs as docs_gen
        docs_path = docs_gen.generate_combined(loaded_schemas, docs_dir / "gatt_services.html")
...
def compile(
    ...
    if schema:
        try:
            _compile_single_schema_mode(schema, output, header, source, docs, config)
        except Exception as e:
    ...
    try:
        if use_combined:
            _compile_combined_mode(schemas, output_dir, header_dir, source_dir, docs_dir, generate_docs)
        else:
            _compile_per_service_mode(schemas, output_dir, header_dir, source_dir, docs_dir, generate_docs, config)
...
def docs(schema: Optional[Path], output: Optional[Path], combined: Optional[bool], per_service: Optional[bool]):
    ...
    if use_combined:
        ...
        try:
            ...
            html_path = docs_gen.generate_combined(schemas_only, out_path)
        ...
        for schema_path, s in loaded_schemas:
            try:
                ...
                html_path = docs_gen.generate(s, out_path)
                click.echo(f"Generated: {html_path}")
            except Exception as e:
...
output:
  ...
  #                                   # false = all services in single gatt_services.html
...
'''
    ...
    click.echo("\nNext steps:")
    click.echo("  1. Edit gatt/echo_service.yaml or create new schemas")
